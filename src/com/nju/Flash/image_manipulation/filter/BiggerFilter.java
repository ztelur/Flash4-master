...
public class BiggerFilter {
	...
	public BiggerFilter(double c, double r) {
		coefficient = c;
		radius = r;
	}
        ...
        for(int col=0;col<width;col++) {
        	 for(int row=0;row<height;row++) {
        		...
        		if (distance < radius*radius) {
        			int newX = (int)((float)(col-centerX)/coefficient+centerX);
        			int newY = (int)((float)(row-centerY)/coefficient+centerY);
